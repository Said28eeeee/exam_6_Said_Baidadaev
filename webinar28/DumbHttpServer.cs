...
namespace webinar28
{
    internal class DumbHttpServer
    {
        ...
        private int _port;
        List<Task> tasks = JsonSerializer.Deserialize<List<Task>>(File.ReadAllText("../../../Tasks.json"));
...

        {
            ...
            {
                if (context.Request.HttpMethod == "POST" && filename.Contains("tasks"))
                {
                    ...
                    Task tas = new Task(int.Parse(rawParams[0]), rawParams[1], rawParams[2], DateTime.Now, rawParams[3], "new");
                    tasks.Add(tas);
                ...
                else if (context.Request.HttpMethod == "GET" && test == ("showTask.html"))
                {
